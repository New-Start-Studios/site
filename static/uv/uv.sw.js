		...
		async fetch({ request: i }) {
			...
			try {
				...
				if (
					...
					(d.headers.origin || (c.meta.url.origin !== e.origin && 'cors' === i.mode)) &&
				...
				if (
					(f.headers.location && (f.headers.location = c.rewriteUrl(f.headers.location)),
				...
				if (
					...
					switch (i.destination) {
						...
						case 'worker':
							{
								const e = [c.bundleScript, c.clientScript, c.configScript, c.handlerScript]
						...
						case 'document':
							(function (t, r = '') {
								return (
									'text/html' === (e.mime.contentType(r || t.pathname) || 'text/html').split(';')[0]
								);
